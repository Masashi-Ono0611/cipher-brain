...
const FIXTURES = [
  ...
  { kind: 'bundled (ANS-104)', id: 'jd4yLGqqJtWJYUpxF8rBzSOMEk0Js1Gh2iE3LQxhGRM',
  ...
  { kind: 'L1', id: 'ww_s6Lhed3hGZnMigPXjQjTzxPwBJj4M5jXdkT4DpSk',
...
try {
  for (const f of FIXTURES) {
    ...
    const env = { ...process.env, CIPHER_BRAIN_AR_HOST: HOST, CIPHER_BRAIN_AR_PORT: PORT, CIPHER_BRAIN_AR_PROTOCOL: PROTOCOL };
    const r = spawnSync('node', [BIN, 'pull', '--locator', f.id, '--backend', 'arweave', '--out', out], { encoding: 'utf8', env });
    if (r.status !== 0) { fail(`${f.kind} ${f.id}: pull failed: ${(r.stderr || '').trim()}`); continue; }
