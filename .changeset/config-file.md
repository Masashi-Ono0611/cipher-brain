...
`CIPHER_BRAIN_MAXSPEND` typo cannot silently drop a spend cap. Only
`CIPHER_BRAIN_*` settings are applied — any other key in the file never enters the
environment, so it cannot reach the processes cipher-brain spawns. Secrets are
allowed, with a warning when the file is group- or other-readable.

`schedule install` is unchanged: it still bakes the values in effect at install
time into the runner, and the runner it generates skips this file entirely — so
editing the file never retunes, or breaks, an already-installed nightly run.
`schedule status` now names the config file it loaded.
