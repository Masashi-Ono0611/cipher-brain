...
try {
  ...
  const PORT = await new Promise((res, rej) => {
    ...
    server.stdout.on('data', (d) => { const m = String(d).match(/READY:(\d+)/); if (m) { clearTimeout(to); res(m[1]); } });
  ...
  await copyFile(distCli, isoBin).catch(() => { throw new Error('dist/cli.mjs not found — run `npm run build` first'); });
  ...
  probe.status === 0 ? pass('control: `arweave` is genuinely unresolvable from the isolated dir')
  ...
  const r = spawnSync('node', [isoBin, 'pull', '--backend', 'arweave', '--locator', loc, '--out', out],
    { env: { ...process.env, CIPHER_BRAIN_AR_PORT: '1', CIPHER_BRAIN_AR_GATEWAYS: `http://127.0.0.1:${PORT}` }, encoding: 'utf8', timeout: 15000 });
  if (r.status !== 0) {
    fail(`gateway pull failed without the arweave package: ${(r.stderr || '').slice(0, 200)}`);
  } else {
    const got = await readFile(out);
    got.equals(bytes) ? pass('gateway pull works with NO arweave npm package (bundled recovery needs only the tx id)')
...
console.log('');
if (failed) { console.log('ARWEAVE NO-DEPS: FAIL'); process.exit(1); }
