...
| [Stripe](https://docs.stripe.com/api/idempotent_requests) | Idempotency keys for a paid API | `snapshot_now`'s `idempotency_key` (the only MCP tool that can spend money): a repeat call carrying the SAME key returns the FIRST call's result instead of re-executing, so an AI agent's own retry logic (a network blip after an arweave/turbo upload already succeeded) cannot spend twice for what it believes is one call. The same-key-must-mean-the-same-request rule ("Keys can only be reused for the exact same operation") is Stripe's too | [#220](https://github.com/Masashi-Ono0611/cipher-brain/issues/220) | Closed | **Shipped** |
