...
dispatcher against each tool's own published schema, so it covers every tool and every
branch, and it names the near miss (`"fille"` → *did you mean file?*). A value the enum
permits but the chosen branch does not need is still accepted and ignored.
