...
| [restic](https://restic.readthedocs.io/en/stable/060_forget.html) / [Perkeep](https://perkeep.org/doc/principles) / [NIST SP 800-88 Rev. 2](https://csrc.nist.gov/pubs/sp/800/88/r2/final) | Retention and forgetting on a store that cannot delete | Three incompatible answers, which is the useful part. restic's **vocabulary**: `forget` (drop the reference) and `prune` (remove the data) are deliberately separate words, and its docs say so outright — a `forget` that sounded like deletion while the ciphertext stayed public would be dangerous. Perkeep's **stance**: "No need to delete in general" — a permanent store may simply say it does not delete, which is a legitimate answer here. NIST's **mechanism**: Cryptographic Erase, sanitize the key rather than the media, made for exactly the media you cannot reach. Today the only erase we can offer is destroying `identity.age`, which shreds every snapshot ever pushed; per-snapshot erasability needs per-snapshot key material and directly opposes the printable recovery kit | [#301](https://github.com/Masashi-Ono0611/cipher-brain/issues/301) | Open | **Undecided — the issue argues three positions** |
